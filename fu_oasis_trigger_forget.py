...
def build_trigger_loader(
    ...
    x_subset = x_test_tensor[indices].clone()
...
def run_trigger_forgetting(
    ...
    The forgetting objective uses "Method A": minimise the probability of predicting the
    attacker's target label for trigger inputs.
    """
    ...
    for _ in range(max(1, num_epochs)):
        for x_trig, _ in trigger_loader:
            x_trig = x_trig.to(device)
            optimizer.zero_grad()
            logits_trigger = model(x_trig)
            probs_trigger = torch.softmax(logits_trigger, dim=1)
            forget_loss = probs_trigger[:, target_label].mean()
