...
def _train_with_optional_anchor(
    model: nn.Module,
    trainloader,
    lr: float,
    num_local_epochs: int,
    num_updates_in_epoch: int,
    anchor_params=None,
    anchor_beta_value: float = 0.0,
):
    """Local training loop with optional anchor regularization."""

    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9)

    if num_updates_in_epoch is not None:
        effective_local_epochs = 1
    else:
        effective_local_epochs = num_local_epochs

    device = next(model.parameters()).device
    if anchor_params is not None:
        anchor_params = [p.to(device) for p in anchor_params]
    ...
    for epoch in range(effective_local_epochs):
        for batch_id, (data, target) in enumerate(trainloader):
            ...
            if anchor_params is not None and anchor_beta_value > 0.0:
                reg_loss = 0.0
                for param, anchor in zip(model.parameters(), anchor_params):
                    reg_loss = reg_loss + torch.sum((param - anchor) ** 2)
                reg_loss = reg_loss * (anchor_beta_value / 2.0)
...
# Anchor-regularized post-learning configuration. Enabled only for the
# asynchronous unlearning branch (Retrain continues to behave as before).
use_anchor_reg = True
anchor_beta = 1e-3
...
# Preserve the unlearned global model as the anchor (w_unlearn) before any
# asynchronous post-learning continues. This anchor is later used only in the
# Unlearn branch to keep the model close to the unlearned solution.
anchor_reference_params = None
if use_anchor_reg and fusion_types_unlearn[1] in unlearned_model_dict:
    anchor_reference_model = OASISNet().to(device)
    anchor_reference_model.load_state_dict(
        copy.deepcopy(unlearned_model_dict[fusion_types_unlearn[1]])
    )
    anchor_reference_model.eval()
    for param in anchor_reference_model.parameters():
        param.requires_grad_(False)
    anchor_reference_params = [
        param.detach().clone() for param in anchor_reference_model.parameters()
    ]
    logging.info("Anchor regularization enabled for the Unlearn branch post-training.")
else:
    logging.info("Anchor regularization disabled or anchor state unavailable.")
    ...
    for fusion_key in fusion_types_unlearn:
        ...
        for party_id in range(1, num_parties):
            ...
            if (
                fusion_key == "Unlearn"
                and use_anchor_reg
                and anchor_reference_params is not None
            ):
                # Encourage post-unlearning updates to stay close to the
                # previously unlearned solution so the model does not drift
                # back toward the erased client's backdoor optimum.
                model_update, party_loss = _train_with_optional_anchor(
                    model=model,
                    trainloader=trainloader_lst[party_id],
                    lr=after_unlearn_lr,
                    num_local_epochs=num_local_epochs_after,
                    num_updates_in_epoch=num_updates_in_epoch_after,
                    anchor_params=anchor_reference_params,
                    anchor_beta_value=anchor_beta,
                )
            else:
                model_update, party_loss = local_training.train(
                    model=model,
