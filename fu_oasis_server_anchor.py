...
def vectorize_model(model: nn.Module) -> torch.Tensor:
    """Detach and flatten a model's parameters."""

    return nn.utils.parameters_to_vector(model.parameters()).detach().clone()


def build_server_unlearn_model(
    model_ref: nn.Module, unlocal_model: nn.Module, lambda_factor: float
) -> nn.Module:
    """Construct the server-side unlearned model via linear interpolation.

    Given the reference model ``model_ref`` (i.e. global model with the erased
    client removed) and the unlearned local model ``unlocal_model`` obtained via
    gradient ascent on the erased data, combine them according to

        w_server = w_ref + lambda_factor * (w_unlocal - w_ref)

    which generalises the Halimi update by allowing a tunable step size toward
    the unlearned local model. ``lambda_factor = 1`` recovers the original
    behaviour where the server fully trusts the unlearned model, whereas values
    in (0, 1) only partially move away from ``w_ref``.
    """

    ref_vec = vectorize_model(model_ref)
    unlocal_vec = vectorize_model(unlocal_model)
    server_vec = ref_vec + lambda_factor * (unlocal_vec - ref_vec)

    server_model = copy.deepcopy(model_ref)
    nn.utils.vector_to_parameters(server_vec, server_model.parameters())
    return server_model


def anchor_regularized_local_training(
    model: nn.Module,
    trainloader,
    anchor_vector: torch.Tensor,
    beta: float,
    num_local_epochs: int,
    num_updates_in_epoch: Optional[int],
    lr: float,
):
    """Local training with anchor regularisation around ``anchor_vector``.

    This mirrors ``LocalTraining.train`` but augments the cross-entropy loss with
    ``(beta / 2) * ||theta - theta_anchor||^2`` to keep the updated weights close
    to the server-side unlearned model.
    """
    ...
    for epoch in range(num_local_epochs):
        for batch_id, (data, target) in enumerate(trainloader):
            ...
            reg_loss = 0.5 * beta * torch.norm(model_vec - anchor_vector) ** 2
...
# Server-side unlearning controls how far the combined model moves toward the
# erased client's gradient-ascent solution. ``lambda_server_unlearn`` = 1.0 uses
# the full unlearned weights, while smaller values keep the model closer to the
# reference weights derived from the poisoned global.
lambda_server_unlearn = 1.0
# Enable/disable anchor regularisation in the post-unlearning FL phase.
use_anchor_reg = True
# Strength of the quadratic anchor penalty. The default 1e-3 biases the local
# objectives mildly toward the server-side unlearned model.
anchor_beta = 1e-3
...
# Store the server-side unlearned model and its flattened parameter vector for
# later use as the anchor in post-unlearning FL.
server_unlearn_anchor_state_dict = None
server_unlearn_anchor_vector = None
...
for fusion_key in fusion_types:
    ...
    # ------------------------------------------------------------------
    # Step 1: build the reference model (server view without the erased party)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Step 2: gradient ascent on the erased client's data (Halimi-style)
    # ------------------------------------------------------------------
    ...
    for epoch in range(num_local_epochs_unlearn):
        ...
        for batch_id, (x_batch, y_batch) in enumerate(trainloader_lst[party_to_be_erased]):
            ...
            if num_updates_in_epoch_unlearn is not None and batch_id >= num_updates_in_epoch_unlearn:
                break
    # ------------------------------------------------------------------
    # Step 3: server-side combination + anchor construction
    # ------------------------------------------------------------------
    unlocal_model = copy.deepcopy(model)
    server_unlearn_model = build_server_unlearn_model(
        model_ref=model_ref,
        unlocal_model=unlocal_model,
        lambda_factor=lambda_server_unlearn,
    )

    server_unlearn_anchor_state_dict = copy.deepcopy(server_unlearn_model.state_dict())
    server_unlearn_anchor_vector = vectorize_model(server_unlearn_model).cpu()
    unlearned_model_dict[fusion_types_unlearn[1]] = copy.deepcopy(server_unlearn_anchor_state_dict)

    eval_model = OASISNet().to(device)
    eval_model.load_state_dict(unlocal_model.state_dict())
    unlearn_clean_acc = Utils.evaluate(testloader_clean, eval_model)
    ...
    eval_model = OASISNet().to(device)
    eval_model.load_state_dict(server_unlearn_anchor_state_dict)
    unlearn_clean_acc = Utils.evaluate(testloader_clean, eval_model)
    print(f"Clean Accuracy for Server Unlearn Model = {unlearn_clean_acc}")
    logging.info(f"Clean Accuracy for Server Unlearn Model = {unlearn_clean_acc}")
    clean_accuracy_unlearn[fusion_types_unlearn[1]] = unlearn_clean_acc
    pois_unlearn_acc = Utils.evaluate(testloader_poison, eval_model)
    print(f"Backdoor Accuracy for Server Unlearn Model = {pois_unlearn_acc}")
    logging.info(f"Backdoor Accuracy for Server Unlearn Model = {pois_unlearn_acc}")
    ...
    for fusion_key in fusion_types_unlearn:
        ...
        for party_id in range(1, num_parties):
            ...
            if (
                fusion_key == "Unlearn"
                and use_anchor_reg
                and server_unlearn_anchor_vector is not None
            ):
                model_update, party_loss = anchor_regularized_local_training(
                    model=model,
                    trainloader=trainloader_lst[party_id],
                    anchor_vector=server_unlearn_anchor_vector,
                    beta=anchor_beta,
                    num_local_epochs=num_local_epochs_after,
                    num_updates_in_epoch=num_updates_in_epoch_after,
                    lr=after_unlearn_lr,
                )
            else:
                model_update, party_loss = local_training.train(
