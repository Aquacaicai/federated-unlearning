...
# -----------------------------------------------------------------------------
# OASIS-specific configuration (IID variant)
# -----------------------------------------------------------------------------
IMAGE_SIZE = 128
NUM_CLASSES = 4
...
def iid_split_indices(train_indices: np.ndarray, train_labels: np.ndarray,
                      num_clients: int) -> List[List[int]]:
    """Split indices IID across clients with equal sizes and broad class coverage."""
    rng = np.random.default_rng(7)
    train_indices = np.array(train_indices)
    train_labels = np.array(train_labels)

    if len(train_indices) != len(train_labels):
        raise ValueError("train_indices and train_labels must have the same length")

    # Compute balanced target sizes so every client sees (almost) the same amount of data.
    base_count = len(train_indices) // num_clients
    remainder = len(train_indices) % num_clients
    target_sizes = [base_count + (1 if cid < remainder else 0) for cid in range(num_clients)]

    per_class_samples = {}
    for cls in range(NUM_CLASSES):
        cls_samples = train_indices[train_labels == cls]
        rng.shuffle(cls_samples)
        per_class_samples[cls] = cls_samples.tolist()

    client_assignments: List[List[int]] = [[] for _ in range(num_clients)]

    # Warm start: ensure each client receives at least one sample per class (when possible).
    for cls in range(NUM_CLASSES):
        for cid in range(num_clients):
            if len(client_assignments[cid]) >= target_sizes[cid]:
                continue
            if per_class_samples[cls]:
                client_assignments[cid].append(per_class_samples[cls].pop())

    leftover = []
    for cls_samples in per_class_samples.values():
        leftover.extend(cls_samples)
    rng.shuffle(leftover)

    cid = 0
    for idx in leftover:
        while len(client_assignments[cid]) >= target_sizes[cid]:
            cid = (cid + 1) % num_clients
        client_assignments[cid].append(idx)
        cid = (cid + 1) % num_clients

    for cid in range(num_clients):
        rng.shuffle(client_assignments[cid])
...
def create_oasis_dataloaders_for_federated(num_parties: int,
    ...
    # Train transform keeps the anatomical structure but introduces light augmentation
    # so that IID clients learn more robustly under asynchronous updates.
    ...
    # Evaluation uses deterministic preprocessing to avoid inflating metrics.
    ...
    client_dataset_indices = iid_split_indices(train_indices, train_labels, num_parties)
...
def run_async_experiment():
    ...
    log_filename = LOG_DIR / f"async_oasis_iid_log_{timestamp}.log"
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        while server.global_version < ASYNC_MAX_VERSION:
            ...
            if (not unlearn_triggered) and current_version >= ASYNC_UNLEARN_TRIGGER:
                ...
                # Post-unlearning evaluation is logged but mini-eval history
                # only tracks strict async checkpoints.
            ...
            # Strictly honor the async evaluation cadence (every eval_interval versions).
            eval_versions_to_run = server.drain_eval_requests()
            for version_to_eval in sorted(eval_versions_to_run):
                if version_to_eval % ASYNC_EVAL_INTERVAL != 0:
                    continue
                if version_to_eval in evaluated_versions:
                    continue
                ...
                evaluated_versions.add(snapshot_version)
    ...
    plt.grid(True)
    plt.legend()
    plot_filename = f"federated-unlearning/doc/images/oasis_async_unlearning_iid_plot_{timestamp}.png"
