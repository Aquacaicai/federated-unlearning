...
# Plotting
# -----------------------------
# Visualization tailored for unlearning analysis
# 1) Clean accuracy of the Unlearn branch before and after unlearning
pre_rounds = np.arange(1, num_fl_rounds + 1)
pre_clean = clean_accuracy["FedAvg"]
post_rounds = np.arange(num_fl_rounds + 1, num_fl_rounds + num_fl_after_unlearn_rounds + 1)
post_clean = clean_accuracy_unlearn_fl_after_unlearn["Unlearn"]

all_rounds = np.concatenate([pre_rounds, post_rounds])
all_clean = np.concatenate([pre_clean, post_clean])

plt.figure()
plt.plot(all_rounds, all_clean, marker="o", linewidth=2, label="Unlearn-Clean Acc (pre+post)")
plt.axvline(x=num_fl_rounds, linestyle="--", linewidth=1.5, label="Unlearning event")
plt.xlabel("Training Rounds")
plt.ylabel("Clean Accuracy (%)")
plt.title("Clean Accuracy of Unlearn Branch Before and After Unlearning")
plt.grid(True)
plt.legend()

image_file_name = f"unlearn_clean_acc_timeline_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
image_file_path = IMAGE_DIR / image_file_name
plt.savefig(image_file_path)
logging.info(f"Clean accuracy timeline plot saved to {image_file_path}")
plt.show()


# 2) Distance between the final Unlearn and Retrain models
...
dist_unlearn_retrain = Utils.get_distance(final_retrain_model, final_unlearn_model)
print(f"Distance between final Unlearn and Retrain models = {dist_unlearn_retrain}")
logging.info(f"Distance between final Unlearn and Retrain models = {dist_unlearn_retrain}")

plt.figure()
plt.bar(["Unlearn vs Retrain"], [dist_unlearn_retrain])
plt.ylabel("Model Distance (L2)")
plt.title("Distance between Final Unlearn and Retrain Models")
plt.grid(axis="y")

distance_image_file_name = f"model_distance_unlearn_vs_retrain_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
distance_image_file_path = IMAGE_DIR / distance_image_file_name
plt.savefig(distance_image_file_path)
logging.info(f"Model distance plot saved to {distance_image_file_path}")
plt.show()
