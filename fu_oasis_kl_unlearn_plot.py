...
# -----------------------------
# Post-processing: distance + plotting
# -----------------------------
# Compute final distance between Unlearn and Retrain models
...
final_distance = Utils.get_distance(final_unlearn_model, final_retrain_model)
print(f"Final distance between Unlearn and Retrain models: {final_distance}")
logging.info(f"Final distance between Unlearn and Retrain models: {final_distance}")

plt.figure()
plt.bar(["Unlearn vs Retrain"], [final_distance])
plt.ylabel("Parameter-space Distance")
plt.title("Final Distance Between Unlearn and Retrain Models")
plt.grid(axis="y")

distance_image_name = f"unlearn_vs_retrain_distance_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
distance_image_path = IMAGE_DIR / distance_image_name
plt.savefig(distance_image_path, bbox_inches="tight")
logging.info(f"Final model distance plot saved to {distance_image_path}")


# Construct clean accuracy curve for Unlearn branch (before vs after)
total_points = num_fl_rounds + num_fl_after_unlearn_rounds
unlearn_clean_curve = np.zeros(total_points, dtype=np.float32)
unlearn_clean_curve[:num_fl_rounds] = clean_accuracy["FedAvg"]
unlearn_clean_curve[num_fl_rounds:] = clean_accuracy_unlearn_fl_after_unlearn["Unlearn"]

rounds = np.arange(1, total_points + 1)
plt.figure()
plt.plot(
    rounds,
    unlearn_clean_curve,
    marker="o",
    linewidth=2,
    label="Unlearn (Clean Accuracy)",
)
unlearn_event_x = num_fl_rounds + 0.5
plt.axvline(x=unlearn_event_x, linestyle="--", linewidth=2, label="Unlearning Event")
plt.xlabel("Global Training Rounds (Pre + Post Unlearning)")
plt.ylabel("Clean Accuracy (%)")
plt.title("Unlearn Branch: Clean Accuracy Before and After Unlearning")
plt.grid(True)
plt.ylim([0, 100])
plt.xlim([1, total_points])
plt.legend()

image_file_name_curve = f"unlearn_clean_curve_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
image_file_path_curve = IMAGE_DIR / image_file_name_curve
plt.savefig(image_file_path_curve, bbox_inches="tight")
logging.info(f"Unlearn clean accuracy curve saved to {image_file_path_curve}")


# Existing comparison plot for post-unlearning FL
