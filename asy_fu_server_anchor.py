...
def build_server_unlearn_model(model_ref, unlocal_model, lambda_factor=LAMBDA_SERVER_UNLEARN):
    """Construct the server-side unlearned model via linear interpolation."""
    server_model = copy.deepcopy(model_ref)
    ref_vec = nn.utils.parameters_to_vector(model_ref.parameters()).detach()
    unlocal_vec = nn.utils.parameters_to_vector(unlocal_model.parameters()).detach()
    combined_vec = ref_vec + lambda_factor * (unlocal_vec - ref_vec)
    nn.utils.vector_to_parameters(combined_vec, server_model.parameters())
    return server_model
...
class UnlearnWorker(threading.Thread):
    ...
    def run(self):
        ...
        server_unlearn_model = build_server_unlearn_model(model_ref, model, LAMBDA_SERVER_UNLEARN)
        logging.info(f"[UNLEARN] Server anchor interpolation applied (lambda={LAMBDA_SERVER_UNLEARN})")
        unlearned_model_state = copy.deepcopy(server_unlearn_model.state_dict())
...
if RUN_MODE == 'sync':
    ...
    for fusion_key in fusion_types:
        ...
        server_unlearn_model = build_server_unlearn_model(model_ref, model, LAMBDA_SERVER_UNLEARN)
        logging.info(f"[UNLEARN] Server anchor interpolation applied during sync mode (lambda={LAMBDA_SERVER_UNLEARN})")
        unlearned_model = copy.deepcopy(server_unlearn_model)
